...
df1 = pd.DataFrame(np.zeros((20,20)))
df2 = pd.DataFrame(np.zeros((20,20)))
df3 = pd.DataFrame(np.zeros((20,20)))
for delt_t in range(0,20):
	...
	for i in range(0, 20 - delt_t):
		...
		p = 0
		q = 0
		print 'today lower limit is', len(sus_lower[i])
		...
		print q,'stocks lower limit today in this network and has successors'
		if q != 0:
		...
		df2.iloc[delt_t, i] = float(q)/len(sus_lower[i])
		df3.iloc[delt_t, i] = float(p)/len(sus_lower[i])
		
	
os.chdir('/Users/shine/work_hard/financial_network/data/threshold/stock_alpha/0.95_2015-1')
df1.to_excel('successors_sus_lower_rate.xlsx', sheet_name='Sheet1')
df2.to_excel('sus_lower_has_successors.xlsx', sheet_name='Sheet1')
df3.to_excel('sus_lower_in_network_successors.xlsx', sheet_name='Sheet1')
