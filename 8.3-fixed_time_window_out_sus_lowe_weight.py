...
df = pd.DataFrame(np.zeros((20,20)))
			...
			if node1 in V.nodes():#确认跌停的这只股票在网络中
				...
				if d:#确定跌停的这只股票有predecessors
					...
					for node2 in d:
						...
						xx = V.get_edge_data(node1, node2)[0]['weight']
						#print xx
						ws = ws+ xx #注意predeceessors和successors的node1、node2顺序是不一样的
...
df.to_excel('successors_sus_lower_rate_weight.xlsx', sheet_name='Sheet1')
