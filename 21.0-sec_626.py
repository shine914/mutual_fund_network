...
# print last.columns
# last_today = last[['node','26-Jun-15']]
# last_today.columns = ['node','26-Jun-15_last']
last_today = last[['node',day+'-Jun-15']]
last_today.columns = ['node',day+'-Jun-15_last']
# print last_today .columns

# last_ystd = last[['node','25-Jun-15']]
# last_ystd.columns = ['node','25-Jun-15_last']
last_ystd = last[['node', ystd+'-Jun-15']]
last_ystd.columns = ['node',ystd+'-Jun-15_last']
print 'last has files', len(last)

low = pd.read_csv('day_last_price_reuters_2956.csv')
low = pd.DataFrame(low)
# print low.columns
# low = low[['node','26-Jun-15']]
low = low[['node', day+'-Jun-15']]
# print low
# low.columns = ['node','26-Jun-15_low']
low.columns = ['node', day+'-Jun-15_low']
...
for workingfile in FileList1:
    ...
    its_low = low[low['node'] == filename][day+'-Jun-15_low']
