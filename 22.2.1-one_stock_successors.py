...
# node = '601318.SH'
node = '601166.SH'
filename = date + '_'+ node + '_ %d min lower limit ratio' %delta
...
for i in range(0, int(330/delta)):
    if (i*delta < 120) or (i*delta >= 210):  #把中午11:30-13:00的休市时间排除
        start = datetime.datetime(2017,1,25,9,30,00) + i* datetime.timedelta(minutes = delta) ##这个时间要是当天的时间
        end = datetime.datetime(2017,1,25,9,30,00) + (i+1) * datetime.timedelta(minutes = delta)
...
os.chdir('/Users/shine/work_hard/financial_network/data/2956_sec')
result.to_csv('time_fre_and_one_stock_successors'+date+node+'.csv')
#
# df = pd.read_csv('time_fre_and_one_stock_successors'+date+'.csv')##为了能用strptime把年份日期去掉，必须重新读入文件
# df = pd.DataFrame(df)
# df['time'] = [datetime.datetime.strptime(d, "%Y-%m-%d %H:%M:%S") for d in df['time']]
# df1 = df[(df['time'] <= datetime.datetime(2017,1,25,11,30,00))]
# df2 = df[(df['time'] >= datetime.datetime(2017,1,25,13,00,00))]
...
# # min_price = int(min(df['low']) * 1.05) #如果是负数 就得乘，这样才能让所有点在取值范围内；如果是正数，就要除；
# min_price = -0.11
#
#
# '''plot'''
...
# ax1.plot_date(df1['time'], df1['frequency'],linestyle='-', xdate=True, ydate=False, color = 'b')
...
# ax1.set_ylim(0, max_fre)  #让两张图的坐标轴一致
# ax1.set_ylabel('time interval lower limit number', color='b')
# ax1.tick_params('time interval lower limit number', colors='b')
#
# ax2 = ax1.twinx()
# ax2.plot_date(df1['time'], df1['low'],linestyle='--', xdate=True, ydate=False, color = 'r')
# ax2.set_ylim(min_price, max_price)  #让两张图的坐标轴一致
# plt.xlabel('time')
# fig.autofmt_xdate() #automatically rotates dates appropriately for you figure.
#
#
# ax1 = plt.subplot(122)  #下午的
# ax1.plot_date(df2['time'], df2['frequency'],linestyle='-', xdate=True, ydate=False, color = 'b')
# datemax = datetime.datetime.strptime('2015-1-10 15:00:00', "%Y-%m-%d %H:%M:%S").time()
...
#
# ax2 = ax1.twinx()
# ax2.plot_date(df2['time'], df2['low'],linestyle='--', xdate=True, ydate=False, color = 'r')
# ax2.set_ylabel('lowest return in 10min', color='r')
# ax2.tick_params('lowest return in 10min', colors='r')
# ax2.set_ylim(min_price, max_price)  #让两张图的坐标轴一致
# plt.xlabel('time')
# fig.autofmt_xdate() #automatically rotates dates appropriately for you figure.
#
