...
for workingfile in FileList:	
	...
	for node1 in lower:
		if node1 in V.nodes():#确认跌停的这只股票在网络中
			lower_within.append(node1)
			#print node1
	
	print 'lenght of today lower is ',len(lower)
	print 'lenght of today lower in the network is ',len(lower_within)


	Z = V.subgraph(lower_within)
	#X = V.subgraph(sus)
	#Y = V.subgraph(sus_lower)
	...
	#nx.write_graphml(Z,filename+"_lower.graphml")

os.chdir('/Users/shine/work_hard/financial_network/data/threshold/stock_alpha/0.95_2015-1/subgraph/lower')
#os.chdir('/Users/shine/work_hard/financial_network/data/threshold/stock_alpha/0.95_2015-1/subgraph/sus_lower')
graph_basic_info.to_excel('lower_subgraph_basic_info-by_date.xlsx', sheet_name='Sheet1')
